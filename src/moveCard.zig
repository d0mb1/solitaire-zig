...
pub fn flipCard() void {
    ...
    while (!main.top_field[row_in_stack_9][stack_9_column].isJoker()) : (row_in_stack_9 += 1) {}

    // If there's no cards in stack 8 all cards in stack 9 go back to stack 8
    // and get covered again
    if (main.top_field[0][0].isJoker()) {

        // if there's also no cards in stack 9 do nothing
        if (main.top_field[0][stack_9_column].isJoker()) {
            return;
        }
        const num_of_cards_in_stack_9: usize = row_in_stack_9;
        for (0..num_of_cards_in_stack_9) |value| {
            row_in_stack_9 -= 1;
            main.top_field[value][0] = main.top_field[row_in_stack_9][stack_9_column];
            main.top_field[value][0].visible = false;
            main.top_field[row_in_stack_9][stack_9_column].value = .joker;
        }
        main.moves += 1;
        ...
        while (!main.top_field[row_in_stack_8][0].isJoker()) : (row_in_stack_8 += 1) {}
        main.top_field[row_in_stack_9][stack_9_column] = main.top_field[row_in_stack_8 - 1][0];
        main.top_field[row_in_stack_9][stack_9_column].visible = true;
        main.top_field[row_in_stack_8 - 1][0].value = .joker;
        main.moves += 1;
    }
}
...
pub fn findExactCardBottom(column: u8, val: u8) u8 {
    var row: usize = 0;
    while (@intFromEnum(main.bottom_field[row][column].value) != val or main.bottom_field[row][column].visible != true) : (row += 1) {
...
pub fn b2bMove(row_from: u8, column_from: u8, column_to: u8) void {
    ...
    // put our chosen card
    var row_to: usize = 0;
    while (!main.bottom_field[row_to][column_to].isJoker()) : (row_to += 1) {}

    // finds out how mave cards should move
    var amount_of_cards: usize = 0;
    while (!main.bottom_field[row_from + amount_of_cards][column_from].isJoker()) : (amount_of_cards += 1) {}

    // if we're trying to place a card on a stack with no cards and the card
    // isn't a king return
    if (row_to == 0 and !main.bottom_field[row_from][column_from].isValue(.king)) {
        return;
    }
...
fn b2bMoveRun(row_from: u8, column_from: u8, amount_of_cards: usize, row_to: usize, column_to: u8) void {
    ...
    if (row_from != 0) {
        main.bottom_field[row_from - 1][column_from].visible = true;
    }
    ...
    while (index < amount_of_cards) : (index += 1) {
        ...
        main.bottom_field[row_from + index][column_from].value = .joker;
    }
    main.moves += 1;
}
...
fn validMoveCheck(card_from: main.Card, card_to: main.Card) bool {
    if (card_to.isOneBiggerThen(card_from) and card_from.isRed() != card_to.isRed()) {
        return true;
    }
    return false;
}
...
pub fn t2bMove(column_from: u8, column_to: u8) void {
    ...
    if (row_to == 0 and !main.top_field[row_from - 1][column_from].isValue(.king)) {
        return;
    }
    ...
    if (row_to == 0 and main.top_field[row_from - 1][column_from].isValue(.king)) {
        ...
        main.top_field[row_from - 1][column_from].value = .joker;
        main.moves += 1;
        return;
    }
...
pub fn findFirstCardBottom(column: u8) u8 {
    var row: usize = 0;
    while (!main.bottom_field[row][column].isJoker()) : (row += 1) {}
    return @intCast(row);
}
...
fn findFirstCardTop(column: u8) u8 {
    var row: usize = 0;
    while (!main.top_field[row][column].isJoker()) : (row += 1) {}
    return @intCast(row);
}
...
pub fn b2finalMove(column_from: u8) void {
    ...
    for (2..main.top_field[0].len) |column_to| {
        if (main.bottom_field[row_from - 1][column_from].isSameShapeAs(main.top_field[0][column_to]) or main.top_field[0][column_to].isJoker()) {
    ...
    if (main.bottom_field[row_from - 1][column_from].isValue(.ace)) {
        main.top_field[row_to][final_column] = main.bottom_field[row_from - 1][column_from];
        main.bottom_field[row_from - 1][column_from].value = .joker;
        main.moves += 1;

        // if it's not the only card in the stack uncover the card underneath it
        if (row_from != 1) {
            main.bottom_field[row_from - 2][column_from].visible = true;
        }
        return;
    }
    if (row_to == 0) return;
    ...
    if (main.bottom_field[row_from - 1][column_from].isOneBiggerThen(main.top_field[row_to - 1][final_column])) {
        main.top_field[row_to][final_column] = main.bottom_field[row_from - 1][column_from];
        main.bottom_field[row_from - 1][column_from].value = .joker;
...
pub fn t2finalMove() void {
    ...
    var final_column: usize = 0;
    for (2..main.top_field[0].len) |column_to| {
        if (main.top_field[row_from - 1][strack9column].isSameShapeAs(main.top_field[0][column_to]) or main.top_field[0][column_to].isJoker()) {
            final_column = column_to;
            break;
        }
    }
    ...
    // if the card is an ace we don't have to check the value of the card underneath
    if (main.top_field[row_from - 1][strack9column].isValue(.ace)) {
        main.top_field[row_to][final_column] = main.top_field[row_from - 1][strack9column];
        main.top_field[row_from - 1][strack9column].value = .joker;
        main.moves += 1;
        return;
    }
    if (row_to == 0) return;
    ...
    // check if it's valid move
    if (main.top_field[row_from - 1][strack9column].isOneBiggerThen(main.top_field[row_to - 1][final_column])) {
        main.top_field[row_to][final_column] = main.top_field[row_from - 1][strack9column];
        main.top_field[row_from - 1][strack9column].value = .joker;
        main.moves += 1;
    }
